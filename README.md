...
- Контроль накопившейся очереди (`GetWebhooksCount`): после включения уведомлений в очереди могут оказаться тысячи событий по всему аккаунту, и новые ответы придут только после них. Приложение показывает размер очереди и позволяет пропустить старые уведомления (`ClearWebhooksQueue`).
